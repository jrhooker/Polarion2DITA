...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="h6">        
        <xsl:variable name="heading-count"
            select="count(preceding-sibling::h6) + 1"/>
    ...
    <xsl:template name="process-siblings">
        ...
        <xsl:choose>
            <xsl:when test="self::h6"></xsl:when>
            <xsl:when
                test="count(preceding-sibling::h6) = number($heading-node)">               
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
                    <xsl:apply-templates/>
                </xsl:copy>
    ...
    <xsl:template
        match="*[preceding-sibling::h6 and not(self::h6)]"/>
